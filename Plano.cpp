...
Plano::Plano() {
    ...
    for (int i=0 ; i < X_SIZE ; i++)
        for (int j=0 ; j < Y_SIZE ; j++){
            ...
            this->plano2d[i][j].setFh(1+rand()%(3-1));
        }
...
int Plano::getfH(Coordenada *coordenada) {
    return this->plano2d[coordenada->getX()][coordenada->getY()].getFh();
}

void Plano::setPosicionObstaculizada(Coordenada *coordenada) {
    this->plano2d[coordenada->getX()][coordenada->getY()].setEsObstaculo(true);
}
